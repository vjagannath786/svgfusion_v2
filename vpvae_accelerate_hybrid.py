...
class VPVAEEncoder(nn.Module):
    def __init__(self, num_element_types, num_command_types,
        ...
        self.svg_input_feature_dim = element_embed_dim + command_embed_dim + num_other_continuous_svg_features
    ...
    def forward(self, svg_matrix_hybrid, pixel_embedding, svg_padding_mask=None, pixel_padding_mask=None):
        ...
        other_continuous_params = svg_matrix_hybrid[:, :, 2:].float() # [B, L_svg, N_other_cont_params]
...
class VPVAEDecoder(nn.Module):
    def __init__(self, num_element_types, num_command_types, num_other_continuous_params_to_reconstruct, 
                 latent_dim, d_model, num_layers, num_heads, max_seq_len):
        super().__init__()
    ...
    def forward(self, z, target_len):
        ...
        return element_type_logits, command_type_logits, predicted_other_continuous_params
...
def vp_vae_hybrid_loss(
    element_logits, command_logits, predicted_other_continuous_params,
    ...
    ce_elem_loss_weight=1.0, ce_cmd_loss_weight=1.0, mse_cont_loss_weight=1.0
):
    ...
    target_other_continuous_params = target_svg_matrix_hybrid[:, :effective_len, 2:].float()
    
    element_logits_eff = element_logits[:, :effective_len, :]
    command_logits_eff = command_logits[:, :effective_len, :]
    predicted_other_continuous_params_eff = predicted_other_continuous_params[:, :effective_len, :]
    ...
    total_loss = (ce_elem_loss_weight * ce_loss_elem +
                  ...
                  mse_cont_loss_weight * mse_loss_cont +
                  kl_weight * kl_loss)
    
    return total_loss, ce_loss_elem, ce_loss_cmd, mse_loss_cont, kl_loss
...
def main():
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(project_name="vp-vae-hybrid-loss", config=config_dict) # New project name
...
if __name__ == "__main__":
    main()
# --- END OF FILE vpvae_accelerate_hybrid.py ---
